...
func (m *mockDao) AddDevice(device *DevicePayload) (primitive.ObjectID, error) {
	m.calledTimes++
	return m.returnValue, m.returnErr
}

func (m *mockDao) GetDevice(id string) (*Device, error) {
...
func Test_GetDevice_GivenDeviceId_ServiceReturnsDeviceObject(t *testing.T) {
	...
	dev, err := out.GetDevice(id.String())
...
func Test_GetPaginatedDevices_GivenList_ServiceReturnsList(t *testing.T) {
	...
	devices, err := out.GetPaginatedDevices(0, 0)
